...
const LTADependents = (props) => {
	...
	useEffect(() => {
		...
		if (props.selectedMode !== '' && props.selectedMode !== undefined && props.selectedMode !== null){
	...
	return (
		<View style = {{borderWidth: 2,
			...
			{props.dependents.map((item, index) => {
				...
				return (
					<View style={{ flexDirection: 'row', alignItems: 'center', borderWidth: 0.5, borderColor: 'grey', flex: 1 }}>
            ...
            <Dropdown title="Mode Of Travel"
                disabled={disable ? true : false}
                forwardedRef={this.travelModeRef}
                dropDownData={modes.map((value)=>value.DisplayText)}
                dropDownCallBack={(index,value) => {setTravelMode(index,value);}} />
		</View>
	);
};
