...
export const ExpenseTypes = (props) => {
  ...
  const resetExpense = () => {
    if (
      this.expenseTypeRef.current !== null &&
      this.expenseTypeRef.current !== undefined
    ) {
  ...
  const updateExpense = (data) => {
    if (
      ...
      let expenseObj = props.expenseTypes.find(
        (el) => el.ID == data.TypeofExpense
      );
      if (!_.isEmpty(expenseData)) {
        setExpense(expenseObj);
      }
      console.log("Expense types : ", props.expenseTypes);
      let index = props.expenseTypes.findIndex(
        (element) => element.ID == data.TypeofExpense
      );
    ...
    <View style={styles.container}>
      <ImageBackground style={styles.cardBackground} resizeMode="cover">
        <Dropdown
          ...
          dropDownData={props.expenseTypes.map((value) => value.DisplayText)}
          dropDownCallBack={(index, value) =>
            onExpenseTypeSelection(index, value)
          }
        />
      </ImageBackground>
    </View>
