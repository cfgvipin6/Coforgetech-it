...
import { ImageBackground,Text,View, TouchableOpacity, ProgressBarAndroidComponent, DatePickerIOS } from 'react-native';
...
const onIncidentalTypeSelection = (index, value)=>{
    let typeObject =   empData.UsIncidentalTypes.find((el)=>el.DisplayText == value);
...
const oncurrencySelection = (index, value)=>{
  ...
  let currencyObject =   empData.UsVoucherCurrencyList.find((el)=>el.DisplayText == value);
...
const onPayTypeSelection = (index, value)=>{
  ...
  let paytypeObject =   empData.UsVoucherPayTypeList.find((el)=>el.DisplayText == value);
  ...
  const onPurposeSelection = (index, value)=>{
        ...
        let purposeObj =   empData.MobileVoucherPurpose.find((el)=>el.DisplayText == value);
  ...
  const resetExpense = ()=>{
    ...
    if (this.currencyRef.current !== null && this.currencyRef.current !== undefined){
      this.currencyRef.current.select(-1);
      setCurrency({});
    }
    if (this.payTypeRef.current !== null && this.payTypeRef.current !== undefined){
    ...
    if (file !== undefined){
        ...
        if (!isNaN(parseInt(file.UsType))){
          ...
          if (this.typeRef.current !== null){
            let expenseTypeIndex = empData.UsIncidentalTypes.findIndex((element) => element.DisplayText == file.TypeofExpenseText);
          ...
          console.log('Setting expense type while update non drop down : ', file.UsType);
          setType(file.UsType);
        }
        ...
        if (this.payTypeRef.current !== null){
          let payTypeIndex = empData.UsVoucherPayTypeList.findIndex((element) => element.DisplayText == file.PayTypeText);
        ...
        if (this.currencyRef.current !== null){
          let currencyIndex = empData.UsVoucherCurrencyList.findIndex((element) => element.DisplayText == file.vc_Currency);
    ...
    return (
        <View style={styles.container}>
          ...
          <ImageBackground style={styles.cardBackground} resizeMode="cover">
            ...
            />
            {expenseId == 6 ? <Dropdown
                title="Type"
                disabled={false}
                forwardedRef={this.typeRef}
                dropDownData={empData.UsIncidentalTypes.map((value)=>value.DisplayText)}
                dropDownCallBack={(index,value) => onIncidentalTypeSelection(index, value)}
                /> : expenseId == 4 ? <LabelEditText
              heading="Type"
              placeHolder="Type"
              myMaxLength={50}
              myNumberOfLines={2}
           ...
           />
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent:'center', flex: 1 }}>
                       <Text>S@</Text>
						<Checkbox.Android disabled={false} color= {appConfig.DARK_BLUISH_COLOR} uncheckedColor="grey" status= {sChecked == true ? 'checked' : 'unchecked'} onPress={() => setSchecked(!sChecked)} />
            <Dropdown
                title="Pay Type"
                disabled={false}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                       <Text>R@</Text>
                       <Checkbox.Android disabled={false} color= {appConfig.DARK_BLUISH_COLOR} uncheckedColor="grey" status= {rChecked == true ? 'checked' : 'unchecked'} onPress={() => setRchecked(!rChecked)} />
            <Dropdown
                title="Currency"
                disabled={false}
