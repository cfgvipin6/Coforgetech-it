...
export const SubmitTo = (props) => {
	...
	let submitToInitialValue = (documentNumber.includes('MBL') || documentNumber.includes('PET') || documentNumber.includes('DRV') || props.subCategoryId == 10
		|| documentNumber.includes('MED') || (empData && empData.IsApplicableToForwardFSO) === 'YES' ? 'FSO' : '');
	...
	const submitToSupervisorView = () => {
		if (submitToValue === 'Supervisor') {
			return <Autocomplete placeholder={'Search Supervisors..'} data={autoSearchSupervisorDataArray} defaultValue={supervisorSearchValue} containerStyle={styles.autocompleteStyle} inputContainerStyle={styles.autocompleteInputStyle} listStyle={styles.autocompleteListStyle} selectTextOnFocus={true} hideResults={autoCompleteSupervisorHideResult} onChangeText={(text) => onAutoCompleteSupervisorChangeText(text)} renderItem={({ item, i }) => renderAutoCompleteSupervisorResult(item, i)} />;
		} else {return null;}
	};
	const fsoView = () => {
		return (
		...
		<View style={styles.userInfoView}>
			<ImageBackground style={styles.cardBackground} resizeMode="cover">
				...
				<View style={styles.cardStyle}>
					{props.documentNumber.includes('MBL') || props.documentNumber.includes('DRV') || props.documentNumber.includes('PET') || props.documentNumber.includes('MED') || (empData && empData.IsApplicableToForwardFSO) === 'YES' || props?.subCategoryId == 10  ? fsoView() :
						<View>
							<Dropdown title={globalConstants.SUBMIT_TO_TEXT} forwardedRef={this.submitToRef} dropDownData={this.submitToOption(props)} dropDownCallBack={(index, value) => onSubmitToSelection(index, value)} />
...
submitToOption = (props) => {
	...
	let checkBand4 =props.myEmpData[0].IsIndianCompany == "1" &&(props.categoryDataId== '1'&& (props.subCategoryId == "0"||props.subCategoryId == "7" || props.subCategoryId == "14")) && props.myEmpData[0].BAND<=4

	let checkBand5 =  props.myEmpData[0].IsIndianCompany == '1' && props.myEmpData[0].BAND == '5' && props.totalAmount > 10000 && props.categoryDataId== '1' || props.categoryDataId =="2" && props.subCategoryId == "0" || props.subCategoryId == "7" || props.subCategoryId == "14"
 
	let checkBand6 = props.myEmpData[0].IsIndianCompany == '1' && props.myEmpData[0].BAND == '6' && props.totalAmount > 50000 && props.categoryDataId== '1' || props.myEmpData[0].Category == '2' && props.subCategoryId == "0" || props.subCategoryId == "7" || props.subCategoryId == "14"

	let checkBand7 = props.myEmpData[0].IsIndianCompany == '1' && props.myEmpData[0].BAND == '7' && props.totalAmount > 75000 && props.categoryDataId== '1' || props.myEmpData[0].Category == '2' && props.subCategoryId == "0" || props.subCategoryId == "7" || props.subCategoryId == "14"
     
	let checkBand8 = props.myEmpData[0].IsIndianCompany == '1' && props.myEmpData[0].BAND == '8' && props.totalAmount > 100000 && props.categoryDataId== '1' || props.myEmpData[0].Category == '2' && props.subCategoryId == "0" || props.subCategoryId == "7" || props.subCategoryId == "14"
 
	 let isSameBand_5 = props.myEmpData[0].BAND == '5' && props.totalAmount<10000 ?  constants.SUBMIT_TO_TYPES :constants.SUBMIT_TO_IND_TYPES
...
else if(props.categoryDataId== '1' && props.subCategoryId=="8" || props.subCategoryId =="9"){
    console.log("else if working")
	return constants.SUBMIT_TO_HR
}
	else{
		 console.log("else working",)
	return checkBand4 ||  checkBand5  ||checkBand6 || checkBand7 || checkBand8 || checkStaffWelfare || checkBand_B? constants.SUBMIT_TO_IND_TYPES: constants.SUBMIT_TO_TYPES
