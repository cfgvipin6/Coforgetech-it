...
import { CostCenterProjectSelection } from "./costCenterProjectSelect.js";
...
export const EmpDetail = (props) => {
  ...
  const [costCenterVal, setCostCenter] = useState(
    emp.CC_CODE + "~" + emp.CC_TXT
  );
    ...
    <View style={styles.container}>
      ...
      <ImageBackground style={styles.cardBackground} resizeMode="cover">
        {props.docNubmer != "" ? (
          <LabelTextDashValue
            heading="Document No#"
            description={props.docNubmer}
          />
        ) : null}
        <LabelTextDashValue
          heading="Employee"
          description={emp.EMPNO + " : " + emp.NAME.trim()}
        />
        <LabelTextDashValue
          heading="Personnel Area"
          description={emp.PA.trim() + " : " + emp.PATXT.trim()}
        />
        <LabelTextDashValue
          heading="Company Code"
          description={emp.CO_CODE.trim() + " : " + emp.CO_TXT.trim()}
        />
        ...
        <LabelTextDashValue
          heading="Personnel Sub Area"
          description={emp.PSA.trim() + " : " + emp.PSATXT.trim()}
        />
        <LabelTextDashValue
          heading="Organisation Unit"
          description={emp.OUTXT.trim()}
        />
        {props.emp && props.emp.length > 0 && (
