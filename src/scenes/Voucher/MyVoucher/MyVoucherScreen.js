...
const stopMessage = "India Expense Claim (Vouchers) modification process has been stopped due to financial year end closing.";
class MyVoucherScreen extends Component {
    ...
    static getDerivedStateFromProps = (nextProps, state) => {
		if (nextProps.myRequestData.length > 0 && !state.query.length > 0) {
			let data = nextProps.myRequestData.filter((data)=> data.VoucherTypeDesc != "CASH - Covid-vaccination")
			return {
				myRequestSearchData: data.sort((a,b)=> new Date(b.DocStartDate) - new Date(a.DocStartDate)),
				localMyRequestSearchData: data.sort((a,b)=> new Date(b.DocStartDate) - new Date(a.DocStartDate)),
			};
		  } else {
			return null;
		  }
	...
	proceedRequest=(item)=>{ //usVoucher handle
		...
		if(docStatusCode == 0 || docStatusCode == 1 || docStatusCode == 5 || docStatusCode == 7 || docStatusCode == 12 || docStatusCode == 14) {
		...
		if(item.DocNo.includes("MBL")){
			console.log("Going to MBL")
              this.props.navigation.navigate("CreateGenericVoucher",{isComingFromMyVoucher: true, docDetails: item,"Title": globalConstants.CREATE_MOBILE_VOUCHER, "CatID" : "6", isFileRequired :item.FileRequired })
		}
		else if(item.DocNo.includes("LTA")){
				this.props.navigation.navigate("CreateLtaVoucher",{isComingFromMyVoucher: true, docDetails: item,"Title": globalConstants.CREATE_LTA_VOUCHER, "CatID" : "3", isFileRequired :item.FileRequired })
		}
		else if(item.DocNo.includes("PET")){
		
               this.props.navigation.navigate("CreateGenericVoucher",{isComingFromMyVoucher: true, docDetails: item,"Title": globalConstants.CREATE_PETROL_VOUCHER, "CatID" : "5", isFileRequired :item.FileRequired })
		}
		else if(item.DocNo.includes("DRV")){
		
				this.props.navigation.navigate("CreateGenericVoucher",{isComingFromMyVoucher: true, docDetails: item,"Title": globalConstants.CREATE_DRIVER_VOUCHER, "CatID" : "4", isFileRequired :item.FileRequired })
		}
		else if(item.DocNo.includes("MED")){
				console.log("Going to MED")
				this.props.navigation.navigate("CreateGenericVoucher",{isComingFromMyVoucher: true, docDetails: item,"Title": globalConstants.CREATE_HEALTH_VOUCHER, "CatID" : "7", isFileRequired :item.FileRequired })
		}
		else if(item.DocType == "TRV" || item.DocType == "REL" || item.DocType == "OTH"){
			console.log("Going to US Voucher")
			if (isDocumentSaved) {
				
					this.props.navigation.navigate("CreateVoucher", {isComingFromMyVoucher: true, docDetails: item })
			} else {
				this.props.navigation.navigate("CreateUSVoucher",{isComingFromMyVoucher: true, docDetails: item,"Title": globalConstants.UPDATE_VOUCHER_TITLE, "CatID" : item.Category, isFileRequired :item.FileRequired })
			}
		} else {
				this.props.navigation.navigate("CreateVoucher2",{isComingFromMyVoucher: true, docDetails: item, isFileRequired :item.FileRequired })
		}
	}
	renderItems = (item, index) => {
		return (
			<ImageBackground style={styles.cardBackground} resizeMode="cover">
				<LabelTextDashValue item = {item} hyperLink = {true} onHyperLinkClick = {(item)=>this.proceedRequest(item)} heading="Document Number" description={item.DocNo} />
				<LabelTextDashValue heading="Voucher Type" description={item.VoucherTypeDesc} />
				<LabelTextDashValue heading="Amount" description={parseFloat(item.TotalApprovedAmt).toFixed(2)} />
				<LabelTextDashValue heading="Document Date" description={item.DocStartDate} />
...
const mapStateToProps = (state) => {
	return {
		...
		myRequestData: state && state.myVoucherReducer && state.myVoucherReducer.myVoucherData,
		myRequestLoading: state && state.myVoucherReducer && state.myVoucherReducer.myVoucherLoading,
		myRequestError: state && state.myVoucherReducer && state.myVoucherReducer.myVoucherError,
	}
}
