...
class ITPendingDetails extends Component {
	...
	componentDidUpdate() {
        if (this.props.itDeskError && this.props.itDeskError.length > 0 && this.state.actionMessage === ''){
            setTimeout(()=>{
              this.setState({actionPopUP:true,actionMessage:this.props.itDeskError});
            },1000);
		  }
		else if (this.props.itDeskActionData && this.props.itDeskActionData.length > 0 && this.state.actionMessage === ''){
            setTimeout(()=>{
              this.setState({actionPopUP:true,actionMessage:this.props.itDeskActionData[0].message});
            },1000);
          }
    }
	componentDidMount() {
	...
	showPopUp= () => {
		writeLog('Dialog is open with exception ' + this.props.itDeskError + ' on ' + 'ITPendingDetails');
		let msgToShow = this.state.actionMessage === 'success' ? REQUEST_SUCCESS : this.props.itDeskError;
		return (
			<UserMessage
				modalVisible={true}
    ...
    takeAction=()=>{
        ...
        this.props.approveRecords(this.props.loginData.SmCode,this.props.loginData.Authkey, this.state.pendingRecord,this.state.remarks,this.state.action);
    }
    ...
    >
				...
				/>
				<ImageBackground style={styles.cardDetailBackground} resizeMode="cover">
					<LabelTextDashValue
					...
					/>
					<LabelTextDashValue heading="Pending Since" description={this.state.pendingRecord.ModifiedOn} />
					<LabelTextDashValue
						heading="Description"
						description={this.state.pendingRecord.RequestDescription}
					/>
				</ImageBackground>
				<Remarks onchangeText={(text) => this.onRemarks(text)} scrollRef={(r) => { this._textInputRef = r; }} />
				<CustomButton
	...
	render() {
		return (
			<View style={styles.detailContainer}>
			 <KeyboardAwareScrollView getTextInputRefs={() => { return [this._textInputRef];}} keyboardShouldPersistTaps="never" >
               {this.renderMainView()}
...
const mapStateToProps = (state) => {
	return {
	loading: state && state.itDeskPendingReducer && state.itDeskPendingReducer.itDeskLoading,
     loginData: state && state.loginReducer && state.loginReducer.loginData,
	 itDeskError: state && state.itDeskPendingReducer && state.itDeskPendingReducer.itDeskError,
	 itDeskActionData: state && state.itDeskPendingReducer && state.itDeskPendingReducer.itDeskActionData,
    };
};
...
const mapDispatchToProps = (dispatch) => {
	return {
     ...
     approveRecords: (empID,authKey,record,remarks,action)=> dispatch(approveITPendingRecord(empID,authKey,record,remarks,action)),
    };
};
