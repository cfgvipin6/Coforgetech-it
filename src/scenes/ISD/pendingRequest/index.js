...
import { getITServicePendingRecords, resetITPending, resetITDesk, getITHistory } from './pendingRequestAction';
...
class PendingRequestScreen extends Component {
    ...
    componentDidUpdate() {
        if (this.props.itError && this.props.itError.length > 0 && this.state.actionMessage === ''){
            setTimeout(()=>{
	...
	handleBack = () => {
		this.setState({localITSearchData:[],actionPopUP: false, actionMessage: ''},()=>{
	...
	openNewPanel = (item, panel) => {
		...
		this.props.getHistoryData(this.props.loginData.SmCode,this.props.loginData.Authkey,item.RequestID);
	...
	expand_collapse_Function = (i) => {
		...
		console.log('state index  : ', this.state.active_index);
	...
	renderItems = (item, index) => {
		return (
	<BoxContainer>
				<LabelTextDashValue heading="Service Number" description={item.ServiceNumber} />
				<LabelTextDashValue heading="Requester Code" description={item.RequesterName} />
	...
	showPopUp= () => {
		writeLog('Dialog is open with exception ' + this.props.itError + ' on ' + 'PendingRequest screen');
		return (
			<UserMessage
				modalVisible={true}
				heading="Error"
	...
	render() {
		...
		return (
            ...
            autoCorrect={false}
				...
				{this.props.itHistoryData && <HistoryView historyData={this.props.itHistoryData} forwardedRef={this._panel} isComingFromVoucher={false} visibility={this.state.modalVisible} onClose={() => this.onHistoryClose()}/>}
...
const mapStateToProps = (state) => {
	return {
		...
		itPendingData: state && state.itDeskPendingReducer && state.itDeskPendingReducer.itDeskPendingData,
		itLoading: state && state.itDeskPendingReducer && state.itDeskPendingReducer.itDeskLoading,
		itHistoryData: state && state.itDeskPendingReducer && state.itDeskPendingReducer.itDeskHistory,
		itError: state && state.itDeskPendingReducer && state.itDeskPendingReducer.itDeskError,
	};
};
...
const mapDispatchToProps = (dispatch) => {
	return {
		...
		dispatch(getITServicePendingRecords(loginId, authKey, isPullToRefreshActive)),
		resetITData:()=>dispatch(resetITDesk()),
		getHistoryData:(loginId, authKey,id)=>dispatch(getITHistory(loginId, authKey,id)),
	};
};
