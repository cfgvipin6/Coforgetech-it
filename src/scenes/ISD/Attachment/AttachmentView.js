...
import { uploadIsdAttachment, actionOnFile, downloadHrFile } from './AttachmentActionCreator';
import ImageResizer from 'react-native-image-resizer';
import { deleteHrFile } from '../../hrassist/utils';
var RNFS = require('react-native-fs');
let fileData = [];
let loginData;
const filePicker = async (setData,files,appState,dispatch,setLoading,documentNumber,rowID) => {
	try {
		...
		let ItemID = rowID;
		let File = { uri: res.uri, name: res.name, filename: res.name, type: res.type };
		let tempData = {};
...
export const getISDFiles = ()=>{
 ...
 export const base64File = async(file)=>{
	let promise = new Promise((resolve,reject)=>{
		let data = RNFetchBlob.fs.readStream(file.FileUri, 'base64', 4095).then((ifStream) => {
			ifStream.open();
...
export const uploadIsdFiles = (requestId)=>{
	fileData.map(async(file,index)=>{
		if (file.FileUri !== undefined){
			...
			let data = { uri: file.FileUri, name: file.FileName, filename: file.FileName, type: file.FileType };
...
export const AttachmentView = (props) =>{
	...
	if (fileArray !== undefined && fileArray.length > 0 && files.length === 0 && !anyAction){
		setTimeout(()=>{
			setData(fileArray);
		},1000);
	}
	...
	const deleteFile = (index,file)=>{
		...
		if (fileId !== undefined && fileId !== null){
			if (props?.isComingFrom == 'HR'){
                deleteHrFile(fileId, (successData)=> {
				...
				actionOnFile(loginData,fileId,'delete',setData,index,files,undefined,setLoading);
			}
		} else {
			files.splice(index,1);
            setData(files=>[...files]);
	...
	const downLoadFile = (index,file)=>{
		...
		if (fileId !== undefined && fileId !== null && props?.isComingFrom !== 'HR'){
			actionOnFile(loginData,fileId,'download',setData,index,files,openDownloadedFile,setLoading);
		}
		if (fileId !== undefined && fileId !== null && props?.isComingFrom == 'HR'){
			downloadHrFile(loginData,fileId,'download',setData,index,files,openDownloadedFile,setLoading);
		}
	};

	const openDownloadedFile = (data,fileName)=>{
		console.log('Opening file:',data);
		const DocumentDir = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.ExternalDirectoryPath;
	...
	const handleCamera = (props)=>{
		setAction(true);
		const options = {title: 'Click picture',storageOptions: {skipBackup: true,path: 'images'}};
	...
	const cameraCallBackFromCameraScreen = (data,documentNumber,rowID)=>{
		...
		let fileName = data.fileName === undefined ? data.uri.substring(data.uri.lastIndexOf('-') + 1,data.uri.length) : data.fileName;
		console.log('File name is : ' + fileName);
		ImageResizer.createResizedImage(data.uri,100,100,'JPEG',10).
				then(response2 => {
                    ...
                    <TouchableOpacity
						...
						onPress={() => {
							if (props.disable){
								...
								filePicker(setData,files,appState,dispatch,setLoading,props.docNumber,props.rowId);
							}
						}}>
			...
			<TouchableOpacity
						onPress={() => {
							if (props.disable === true){
								showToast('You can not delete file of submitted record.');
							} else {
								deleteFile(index,file);
							}
						}}>
						<Icon name="delete" size={23} color={appConfig.INVALID_BORDER_COLOR} />
			</TouchableOpacity>
			</View>
            </View>
            );
