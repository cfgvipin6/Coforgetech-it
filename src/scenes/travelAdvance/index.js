...
import { TravelAdvanceFetchData, resetTravelAdvanceStore } from './travelAdvanceAction';
...
export class TravelAdvanceScreen extends Component {
  ...
  componentDidMount() {
    ...
    if (isDTRService) {
      writeLog('Invoked ' + 'fetchTravelAdvanceData' + ' of ' + 'TravelAdvanceScreen' + ' for Domestic');
      this.props.fetchTravelAdvanceData(this.props.empCode, this.props.accessToken, isPullToRefreshActive, 'D');
    } else if (isITRService) {
      writeLog('Invoked ' + 'fetchTravelAdvanceData' + ' of ' + 'TravelAdvanceScreen' + ' for International');
      this.props.fetchTravelAdvanceData(this.props.empCode, this.props.accessToken, isPullToRefreshActive, 'I');
    }
  }

  static getDerivedStateFromProps(nextProps, state) {
    // console.log("next props::::::", nextProps)
    if (nextProps.travelAdvanceData[0] && nextProps.travelAdvanceData.length > 0 && !state.query.length > 0) {
      return {
  ...
  checkServiceType() {
    if (userServiceType != '' && userServiceType != null && userServiceType != undefined) {
      writeLog('Invoked ' + 'checkServiceType' + ' of ' + 'TravelAdvanceScreen' + ' for ' + userServiceType);
  ...
  resetTravelAdvanceState = () => {
    ...
    if (isDTRService) {
      writeLog('Invoked ' + 'fetchTravelAdvanceData for resetTravelAdvanceState' + ' of ' + 'TravelAdvanceScreen' + ' for Domestic');
      this.props.fetchTravelAdvanceData(this.props.empCode, this.props.accessToken, isPullToRefreshActive, 'D');
    } else if (isITRService) {
      writeLog('Invoked ' + 'fetchTravelAdvanceData for resetTravelAdvanceState' + ' of ' + 'TravelAdvanceScreen' + ' for International');
      this.props.fetchTravelAdvanceData(this.props.empCode, this.props.accessToken, isPullToRefreshActive, 'I');
    }
  }
    ...
    this.setState(
      ...
      () => {
        const filteredData = this.state.localTravelAdvanceSearchData.filter(element => {
    ...
    return (
      <View>
          {this.showTravelAdvanceRowGrid(constants.VISIT_TYPE_TEXT, item.VisitType)}
          {this.showTravelAdvanceRowGrid(globalConstants.COST_CENTER_TEXT,item.CostCenterCode.trim() + ' : ' + item.CostCenterDesc.trim())}
          {this.showTravelAdvanceRowGrid(globalConstants.PROJECT_TEXT,item.ProjectDesc === 'NA' ?
            'NA' : item.ProjectCode.trim() + ' : ' + item.ProjectDesc.trim())}
          {this.showTravelAdvanceRowGrid(constants.BILLABLE_TEXT, item.Billable === 'Y' ? 'Yes' : 'No')}
          {this.showTravelAdvanceRowGrid(constants.TOTAL_ADV_TAKEN_TEXT, item.TotalAdvTaken)}
          {this.showTravelAdvanceRowGrid(constants.ADDITIONAL_ADV_REQUIRED_TEXT, item.AddlAdvRequired)}
      </View>
    );
  }
    ...
    return (
      <BoxContainer>
          <View style={styles.cardStyle}>
            {this.showTravelAdvanceRowGrid(globalConstants.DOCUMENT_NUMBER_TEXT,item.DocumentNo.trim())}
            {this.showTravelAdvanceRowGrid(globalConstants.EMPLOYEE_TEXT, item.DocOwnerCode.trim() + ' : ' + item.DocOwnerName.trim())}
            {this.showTravelAdvanceRowGrid(constants.ITINERARY_TEXT, item.Itinerary.trim())}
            {this.showTravelAdvanceRowGrid(constants.DEP_DATE_TEXT, deptDate)}
            {this.showTravelAdvanceRowGrid(constants.EXP_DATE_OF_RETURN_TEXT, returnDate)}
          </View>
          <Seperator/>
            {this.state.active_index === index ? this.renderTravelSubView(item) : <View />}
          <View
            style={{
              flexDirection: 'row',
              width: '100%',
          ...
          {this.state.active_index === index ? (
            ...
            <TouchableOpacity
              onPress={() => this.travelRequestAction(globalConstants.APPROVED_TEXT, item, isDTRService)}
              style={{ alignItems: 'center' }}
            >
            ...
            <TouchableOpacity
              onPress={() => this.travelRequestAction(globalConstants.REJECTED_TEXT, item, isDTRService)}
              style={{ alignItems: 'center' }}
            >
    ...
    return (
      <View style={globalFontStyle.listContentViewGlobal}>
        <FlatList
          ...
          renderItem={({ item, index }) => this.renderAdvTravelRequest(item, index)}
    ...
    return (
      <ImageBackground
       ...
       style={styles.container}>
        ...
        <SubHeader
          pageTitle={(isDTRService) ? globalConstants.TRAVEL_ADVANCE_DOMESTIC_TITLE : globalConstants.TRAVEL_ADVANCE_INTERNATIONAL_TITLE}
          backVisible={true}
          logoutVisible={true}
...
mapDispatchToProps = dispatch => {
  return {
    ...
    fetchTravelAdvanceData: (empCode, authToken, isPullToRefreshActive, serviceType) =>
      dispatch(TravelAdvanceFetchData(empCode, authToken, isPullToRefreshActive, serviceType)),
  };
};
