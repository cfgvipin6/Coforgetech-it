...
export class ExitScreen extends Component {
  ...
  componentDidUpdate() {
    if (
      this.props.exitError &&
      this.props.exitError.length > 0 &&
  ...
  updateSearch = (searchText) => {
    ...
    this.setState(
      ...
      () => {
        const filteredData = this.state.localExitSearchData.filter(
          (element) => {
    ...
    return (
      <BoxContainer>
        <View style={styles.view_One}>
          ...
          {this.showExitRowGrid(globalConstants.MOBILE_TEXT, item.vc_mobile_no)}
          {this.showExitRowGrid(
