...
import { ResignationDetails, RadioForms, Skills ,SkillSearchView, EditTextWithHeading, TreatmentRadioForms} from './ExitHelper';
import RadioForm from 'react-native-simple-radio-button';
import {KeyboardAwareScrollView} from 'react-native-keyboard-aware-scrollview';
import { RESIGNATION_TYPE, RESIGNATION_OPTIONS, RP_RESIGNED, YES_NO_OPTIONS, PREVENT_EMAILS, DENY_INTERNET, PLANNED_SEPARATION, STATUS_OPTIONS, STATUS, SCHEDULED_LWD,NP_TREATMENT_OPTIONS, NP_TREATMENT, REASON_TITLE, REMARKS_NP_RECC, WAIVER_TITLE } from './constants';
import { fetchExitDetails, fetchNoticeDetails, resetDetails, exitActionTaken } from './exitAction';
...
export class ExitDetails extends Component {
  constructor(props){
    ...
    this.state = {
      ...
      resignationType:this.props.navigation.state.params.docDetails.in_rsg_type,
      ...
      separationType:this.props.navigation.state.params.docDetails.ch_plannedseparation,
      resignationDate:this.props.navigation.state.params.docDetails.dt_resignation,
      statusType:1,
      lwd:this.props.navigation.state.params.docDetails.dt_scheduled_lwd === '01-Jan-1900' ? '-' : this.props.navigation.state.params.docDetails.dt_scheduled_lwd,
      ...
      npTreatMent:parseInt(this.props.navigation.state.params.docDetails.in_treatment),
      reasonRemarks:'',
      resignationReason:'',
      waiverRemarks:'',
      shortPeriod:this.props.navigation.state.params.docDetails.in_notice_short,
      supRecomm:this.props.navigation.state.params.docDetails.Sup_NP_Recommendation,
      isNSSEMP:this.props.navigation.state.params.docDetails.ISNSSEMP,
      IsSkillApplicable:this.props.navigation.state.params.docDetails.IsSkillApplicable,
      showModal: false,
      messageType: null,
  ...
  static getDerivedStateFromProps(nextProps, state) {
    if (nextProps.detailData && nextProps.detailData && nextProps.detailData.length > 0 && !state.query.length > 0){
      return {
        skillData: nextProps.detailData[0].skill,
        localSkillData: nextProps.detailData[0].skill,
      };
    } else if (nextProps.noticePeriod !== '' && nextProps.noticePeriod.npshort && parseInt(nextProps.noticePeriod.npshort) <= 0){
  ...
  reasonArrayCallBack=(response)=>{
    ...
    reasonArrayData = response[0].ReasonType.filter((item)=>item.Type == 'Reason');
      this.setState({
        reasonTypeArray:response[0]?.ReasonType.filter((item)=>item.Type == 'ReasonType'),
      },()=>{
        ...
        if (this.reasonTypeRef.current !== null){
          let reasonTypeIndex = this.state.reasonTypeArray.findIndex((item)=> this.state.resignationType == item.ID);
          let reasonTypeData = this.state.reasonTypeArray[reasonTypeIndex];
          this.reasonTypeRef.current.select(reasonTypeIndex);
          let reasonFilteredArray = reasonArrayData?.filter((item)=>item?.ReasonTypeID == reasonTypeData?.ID);
          this.setState({reasonArray : reasonFilteredArray},()=>{
            let reasonIndex = this.state.reasonArray?.findIndex((item)=> this.state.reasonType == item.ID);
  ...
  onFocus = () => {
      this.props.fetchDetails(this.props.empCode,this.props.accessToken,this.state.exitItem.in_ecc_no, this.reasonArrayCallBack);
  };
  componentDidUpdate(prevState,prevProps){

     if (this.state.resultModalVisible !== true && this.props.exitActionError.length > 0){
       setTimeout(()=>{
  ...
  pickSkill = skillData => {
    writeLog('Invoked ' + 'pickSkill' + ' of ' + 'ExitDetails' + ' for skill ' + skillData.ID);
    this.modalClose();
    ...
    this.setState({
      ...
      setTimeout(()=>{
        this.props.fetchNoticePeriod(this.props.empCode,this.props.accessToken,this.state.resignationDate,this.state.rpType,this.state.selectedSkill,lwdFinal,this.state.resignationType,this.state.requestorId);
      },1000);
  ...
  lwdChanged=(date)=>{
    ...
    // console.log("RP type is ", this.state.rpType);
     this.setState({lwd:date},()=>{
      this.props.fetchNoticePeriod(this.props.empCode,this.props.accessToken,this.state.resignationDate,this.state.rpType,this.state.selectedSkill,this.state.lwd,this.state.resignationType,this.state.requestorId);
     });
  }
   renderDateRow=()=>{
     return (
       <View style={styles.halfHolder}>
         ...
         <DatePicker disabled={true}  callBack={this.lwdChanged} title={this.state.lwd} ref={this.startCalendar} />
       </View>
     );
   }
   backNavigate() {
    this.setState({
      ...
      resignationDate:this.props.navigation.state.params.docDetails.dt_resignation,
      statusType:1,
      lwd:this.props.navigation.state.params.docDetails.dt_scheduled_lwd === '01-Jan-1900' ? '-'.dt_requested_lwd : this.props.navigation.state.params.docDetails.dt_scheduled_lwd,
      rlwd:this.props.navigation.state.params.docDetails.dt_requested_lwd,
      requestorId:this.props.navigation.state.params.docDetails.ch_initiatedby,
  ...
  onReasonTypeSelection = (i, val) => {
      let indexFound = this.state.reasonTypeArray?.findIndex((item)=> item.ReasonText == val);
      this.setState({resignationType: this.state.reasonTypeArray[indexFound].ID},()=>{
      let reasonTypeIndex = this.state.reasonTypeArray?.findIndex((item)=> this.state.resignationType == item.ID);
      let reasonTypeData = this.state.reasonTypeArray[reasonTypeIndex];
      let reasonFilteredArray = reasonArrayData?.filter((item)=>item.ReasonTypeID == reasonTypeData.ID);
      this.setState({reasonArray : reasonFilteredArray, reasonType:''},()=>{
  ...
  onReasonSelection = (i, val) => {
    ...
    let indexFound = this.state.reasonArray.findIndex((item)=> item.ReasonText == val);
  ...
  goAhead = () => {
    ...
    let NP = this.props.noticePeriod.npshort === undefined ? this.state.shortPeriod : this.props.noticePeriod.npshort;
    let data = {};
    data.treatment = this.state.npTreatMent + 1;
    data.RegDate = this.state.resignationDate;
    data.SLWD = this.state.lwd;
    data.npReq = this.props.noticePeriod === '' ? this.state.exitItem.in_notice_reqd : this.props.noticePeriod.strNPreqd;
    ...
    if (this.state.reasonType == '' || this.state.reasonType == undefined || this.state.reasonType == null){
    ...
    } else if (this.state.statusType === 1 && (this.state.npTreatMent === 1 || this.state.npTreatMent === 2 || this.state.npTreatMent === 3) && NP !== '0' && this.state.waiverRemarks === ''){
      return alert(constants.WAIVER_REMARKS);
    } else if (this.state.statusType === 1 && this.state.reasonRemarks === ''){
      return alert(constants.SUPERVISOR_REMARKS);
    }}
    // console.log("Document data is: ",data);
    if (this.state.statusType === 0){
        this.props.completeRequest(this.props.docData[0],2,this.props.empCode,'','',data);
    } else {
  ...
  render() {
    ...
    >
        ...
        />
        <KeyboardAwareScrollView style={styles.container} keyboardShouldPersistTaps="never">
        <ResignationDetails
        ...
        <Dropdown
            ...
            dropDownData={this.state.reasonTypeArray.map((item)=>item.ReasonText)}
            dropDownCallBack={(index, value) => this.onReasonTypeSelection(index, value)}
          />

          <Dropdown
            title={'Reason for Resignation'}
            forwardedRef={this.reasonRef}
            dropDownData={this.state.reasonArray.map((item)=>item.ReasonText)}
            dropDownCallBack={(index, value) => this.onReasonSelection(index, value)}
          />
        ...
        {this.state.IsSkillApplicable === 'Y' ? <Skills skillData={this.state.skillData} skillSelected={this.state.selectedSkill}
              searchCallBack={() => {
                this.onSkillSearch();
              }}/> : null}
        ...
        /> : null}
        <RadioForms selectedVal = {this.state.rpType} onValueSelection={(val)=>this.rp_Resignation(val)} labelHorizontal= {true} title={RP_RESIGNED} options = {YES_NO_OPTIONS}/>
        <RadioForms selectedVal = {this.state.mailType} onValueSelection={(val)=>this.preventMail(val)} labelHorizontal= {true} title={PREVENT_EMAILS} options = {YES_NO_OPTIONS}/>
        <RadioForms selectedVal = {this.state.internetType} onValueSelection={(val)=>this.denyInternet(val)} labelHorizontal= {true} title={DENY_INTERNET} options = {YES_NO_OPTIONS}/>
        <RadioForms exitItem = {this.state.exitItem} selectedVal = {this.state.separationType} onValueSelection={(val)=>this.plannedSeparation(val)} labelHorizontal= {true} title={PLANNED_SEPARATION} options = {YES_NO_OPTIONS}/>
        <RadioForms selectedVal = {this.state.statusType} onValueSelection={(val)=>this.setStatus(val)} labelHorizontal= {false} title={STATUS} options = {STATUS_OPTIONS}/>
        {this.state.statusType === 1 ? this.renderDateRow() : null}


        {((this.state.statusType === 1 && NP !== '0') || (this.state.Sup_NP_Recommendation === 'N' && this.state.isNSSEMP == 'Y')) ? <TreatmentRadioForms selectedVal = {this.state.npTreatMent} onValueSelection={(val)=>this.setNpTreatment(val)} labelHorizontal= {true} title={NP_TREATMENT} options = {NP_TREATMENT_OPTIONS}/> : null}


        {((this.state.npTreatMent === 1 || this.state.npTreatMent === 2 || this.state.npTreatMent === 3) && NP !== '0') ?
        <EditTextWithHeading title={WAIVER_TITLE} reasonRemarks={(remarks)=>this.waiverRemarks(remarks)}/> : null }
        <EditTextWithHeading title={REMARKS_NP_RECC} reasonRemarks={(reasonRemarks)=>this.reasonRemarks(reasonRemarks)}/>
        <TouchableOpacity style={styles.submitButton} onPress={() => this.goAhead()}>
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    fetchNoticePeriod:(empCode, authToken,rn_date,rp_type,skillCode,lwd,resignationCode,requestorId)=>
    dispatch(fetchNoticeDetails(empCode, authToken,rn_date,rp_type,skillCode,lwd,resignationCode,requestorId)),
...
export default connect(mapStateToProps, mapDispatchToProps)(ExitDetails);
