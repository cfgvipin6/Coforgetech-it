...
import { leaveActionCreator2, fetchLeaveHistory } from "./leaveActionCreator";
import {
  APPROVE,
  REJECT,
...
class LeaveScreen extends Component {
  ...
  updateSearch = (searchText) => {
    ...
    this.setState(
      ...
      () => {
        const filteredData = this.state.localLeaveSearchList.filter(
          (element) => {
  ...
  onHistoryData = (data) => {
    if (data?.Leave?.length > 0 || data?.ReversalLeave?.length > 0) {
      this.setState(
        {
          leaveHistory: data?.Leave.length > 0 ? data.Leave : [],
          reversalHistory:
            data?.ReversalLeave?.length > 0 ? data.ReversalLeave : [],
        },
        () => {
          setTimeout(() => {
            if (this._panel) {
