...
class SchemeAndPolicyScreen extends Component {
  ...
  async componentDidMount() {
    ...
    let url = properties.policySchemes.concat(
      "?id=" + this.props.loginData.PolicyID
    );
  ...
  sectionItem(item) {
    return (
      <TouchableOpacity onPress={() => this.sectionItemClick(item)}>
        <View style={{}}>
          <Text style={globalFontStyle.dashboardTitleSize}>{item.Title}</Text>
      ...
      this.setState({ data: [] }, () => {
        this.setState({ data: menuData }, () => {
          setTimeout(() => {
            ...
            this.props.navigation.navigate("SchemeDetails", {
              items: node,
