...
export class CDSScreen extends Component {
  ...
  componentDidMount() {
    ...
    this.props.fetchCdsData(this.props.empCode, this.props.accessToken, isPullToRefreshActive);
  }
  componentDidUpdate(){
    if (this.props.cdsError && this.props.cdsError.length > 0 && this.state.errorMessage === ''){
      setTimeout(()=>{
  ...
  static getDerivedStateFromProps(nextProps, state) {
    ...
    if (nextProps.cdsData[0] && nextProps.cdsData.length > 0 && !state.query.length > 0) {
      return {
    ...
    if (itemValue != '' && itemValue != null && itemValue != undefined && itemValue != ' : ') {
      return (
        <View style={styles.rowStyle}>
          ...
          <Text
            style={
              itemName === constants.UTILIZATION_TEXT && itemValue === 'Non-Budget'
      ...
      <View>
        <ImageBackground style={styles.cardBackground} resizeMode="cover">
          <View style={styles.cardStyle}>
            {this.showCdsRowGrid(globalConstants.DOCUMENT_NUMBER_TEXT, item.CDSCode)}
            {this.showCdsRowGrid(
            ...
            {this.showCdsRowGrid(constants.CDS_STATUS_DESC_TEXT, item.CDSStatusDesc.trim())}
            {this.showCdsRowGrid(
            ...
            {this.showCdsRowGrid(globalConstants.COMPANY_CODE_TEXT, item.CompCode)}
            {this.showCdsRowGrid(constants.UTILIZATION_TEXT, item.UtilizationDesc)}
            {this.showCdsRowGrid(constants.TOTAL_AMOUNT_TEXT, item.CDSFinalAmount)}
          </View>
  ...
  showError = () => {
    ...
    writeLog('Dialog is open with exception ' + this.props.cdsError + ' on ' + 'CDSScreen');
    return (
      <UserMessage
        modalVisible={true}
        heading="Error"
  ...
  openNewPanel = cdsCode => {
    writeLog('Clicked on ' + 'openNewPanel' + ' of ' + 'CDSScreen' + ' for ' + cdsCode);
    ...
    return keys.map((key)=>{
      if (data[key].IsActive){
        return (
          ...
          <Text style={globalFontStyle.cardRightText}>{data[key].Value === '' ? '-' : data[key].Value.replace(':', ' : ')}</Text>
          </View>
         );
      }
    });
...
export default connect(mapStateToProps, mapDispatchToProps)(CDSScreen);
