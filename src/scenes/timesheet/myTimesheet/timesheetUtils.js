...
let arrayContainer = [];
export const addMoreObject = (previousObject,loginData,currentDate,startDate, endDate,dayTypes)=>{
    let pushableObject = {};
    let weekArray = [];
    let weekDays;
    if (previousObject){
...
const generateWeekDays = (currentDay,dayTypes)=>{
  ...
  for (let i = 0; i < dayTypes.length; i++){
    ...
    weekDay.DayID = null;
    weekDay.Key = moment(currentDay).add(i,'d').format('DD-MMM-YYYY');
    weekDay.Status = 0;
...
const filterDataForValidation = async(data,todayDate)=>{
  ...
  data.map((tile)=>{
    if (
      ...
      ]?.DID == 0 || (tile.TodayDate !== undefined && tile.TodayDate !== todayDate)
    ) {
...
export const validateData = (data,todayDate,dayTypes,saveType,projectList)=>{
    let weekDays = data.lstColumns;
    let todayRecords = weekDays.find((item)=>item.Key == todayDate);
    let isWorkingDay =  dayTypes?.find(rec=>rec?.StartDate == todayRecords.Key)?.DayID == 1 || dayTypes?.find(rec=>rec?.StartDate == todayRecords.Key)?.DayID == 5;
      if (
        (isWorkingDay && todayRecords.Value != 0) ||
        (isWorkingDay && todayRecords.ProjectCode != 'NO-PRJ' && projectList.length > 1) ||
      ...
      )
    {
      if (todayRecords.Value !== 0 && todayRecords.Value.trim() !== ''){
        if (todayRecords.ProjectCode !== 'NO-PRJ'){
          if (todayRecords.ActivityCode == 0 || todayRecords.ActivityCode.trim() == ''){
            arrayContainer.push('Please select activity for date : ' + todayDate);
          }
        } else {
            if (projectList.length > 1){
              arrayContainer.push('Please select Project Code for date : ' + todayDate);
            }
        }
      } else {
...
export const validateDataForSubmit = async(data, recordData,saveType,empData,dayTypes)=>{
  let errorArray = [];
  let dataToValidate = await prepareTimeSheetData2(data);
   if (saveType == 'Submit'){
    recordData[0].lstColumns.map((item)=>{
      let isWorkingDay =  dayTypes?.find(rec=>rec?.StartDate == item.Key)?.DayID == 1 || dayTypes?.find(rec=>rec?.StartDate == item.Key)?.DayID == 5;
      let isHalfDay =  dayTypes?.find(rec=>rec?.StartDate == item.Key)?.DayID == 5;
      let dayWiseData =  dataToValidate.filter((data)=>data.TimesheetDate == item.Key);
     let hoursPerDay =  dayWiseData.reduce((a,b)=> a + parseFloat(b.EffortHrs !== '' ? b.EffortHrs : 0),0);
     let mandatoryHours = isHalfDay ? empData.empData[0].MinHourHalfDay : empData.empData[0].MinHourFullDay;
     if (hoursPerDay < mandatoryHours && isWorkingDay){
        errorArray.push('Hours cannot be less than ' + mandatoryHours + ' on ' + item.Key);
     }
  });
  }
    return errorArray;
...
export const validateData4 = async(data,recordData,saveType,empData,dayTypes,currentDay,projectList)=>{
let holder = [];
let todayData = [];
data.map((tile)=>{holder.push(...tile.lstColumns);});
// todayData.push(...holder.filter((item)=>item.Key == currentDay));
todayData.push(...holder);
  arrayContainer = [];
  if (saveType == 'Save'){
    let todayHours =  todayData.reduce((a,b)=> a + parseFloat(b.Value !== '' ? b.Value : 0),0);
    if (todayHours < 1){
...
export const getTodayHours = (recordsResponse,currentDay)=>{
    ...
    return rec.filter(item=>item.TimesheetDate == currentDay).reduce((a,b)=> a + parseFloat(b.EffortHrs !== '' ? b.EffortHrs : 0),0);
  });
};

export const getTodayData = async(recordsResponse,currentDay)=>{
  let dataContaier = [];
   let filteredTile =  recordsResponse.filter((item)=>item?.TodayDate == currentDay);
...
export const getTodayData2 = async(recordsResponse,currentDay)=>{
  let dataContaier = [];
   let filteredTile =   recordsResponse.filter((item)=>item?.TimesheetDate.split(',').filter((item)=> item == currentDay));
...
export const prepareTimeSheetData = async(data)=>{
  ...
  data.map((item,index)=>{
      ...
      for (let i = 0; i < weekDays.length; i++){
          ...
          record.TimesheetFromDate = item.TimesheetFromDate ? item.TimesheetFromDate : item.TimesheetDate?.split(',')[0];
          record.TimesheetToDate =  item.TimesheetToDate ? item.TimesheetToDate : item.TimesheetDate?.split(',')[item.TimesheetDate?.split(',').length - 1];
...
export const prepareTimeSheet = async(data,empData,rejectionRemarks,supervisorCode)=>{
