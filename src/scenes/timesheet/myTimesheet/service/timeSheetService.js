  ...
  if (isNetwork) {
    try {
      ...
      if (type == 'Binding') {
        form.append(
          'EmpCode',
          data.Type === 3 ? data.EmpCode : loginData.SmCode
        );
      }
  ...
  if (isNetwork) {
    try {
      ...
      if (response?.Message || response?.Result) {
        ...
        let responseApproval = await fetchPOSTMethodNew(
          url,
          JSON.stringify(body)
        );
        if (responseApproval?.Message || responseApproval?.Result) {
          successCallBack(
            responseApproval.Message
              ? responseApproval.Message
              : responseApproval.Result,
            submitType
          );
...
export const saveSubmitTimeSheet = async (
  loginData,
  data,
  successCallBack,
  errorCallBack,
...
export const getLineItemHistory = async (
  loginData,
  TID,
  successCallBack,
  errorCallBack
...
export const fetchTimesheetWeek = async (
  year,
  type,
  startDt,
  endDt,
