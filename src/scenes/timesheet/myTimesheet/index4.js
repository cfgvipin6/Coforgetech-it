...
import { fetchTimesheetWeek, fetchEmpDetails, getLineItemHistory,fetchSupervisors } from './service/timeSheetService';
...
const yearData = [String(new Date().getFullYear()),  String(new Date().getFullYear() - 1),  String(new Date().getFullYear() - 2)];
export class MyTimesheetScreen4 extends Component {
  ...
  callLastThreeWeek = () => {
    this.setState({
      ...
      fetchTimesheetWeek(undefined, 9, '', '', this.props.loginData.SmCode).then((resWeek) => { //handle hard coded empCode here
        console.log('fffffff',resWeek);
        this.setState({isLoading:false});
        if (resWeek.Result && resWeek.Result.length > 0) {
          const lastThreeWeekData = resWeek.Result.filter(it => it.Selected === 'N');
          this.setState({
  ...
  fetchHistory = async (rec) =>{
    ...
    if (TID){
      ...
      await getLineItemHistory(this.props.loginData, TID, this.onHistoryResponse, this.onResponseFailure);
    }
  }
        ...
        .then(resEmp => {
          ...
          if (resEmp.Result) {
            this.setState({
              ...
              if (this.state.isWeekChanged && this.weekCalenderRef?.current) {
  ...
  callWeekApi() {
    this.setState({
      ...
      fetchTimesheetWeek(this.state.yearValue, 1, undefined, undefined, undefined).then(resWeek => {
        if (resWeek.Result && resWeek.Result.length > 0) {
          const defaultWeek = resWeek.Result.filter((it, idx) => it.Selected === 'Y').map(it => it.Display);
          this.setState({
            weekArray: resWeek.Result,
            selectedWeek: defaultWeek.length > 0 ? defaultWeek[0]  : resWeek.Result[0].Value,
  ...
  employeeDetailsView = () => {
    ...
    if (empDetailsArray.length > 0) {
      const empName = empDetailsArray[0].EmpCode + ' : ' + empDetailsArray[0].EmpName;
      const approveName = empDetailsArray[0].SupervisorCode + ' : ' + empDetailsArray[0].SupervisorName;
      const compName = empDetailsArray[0].CompanyCode + ' : ' + empDetailsArray[0].CompanyName;
      const statusDesc = empDetailsArray[0].StatusDesc;
      // console.log("check",this.state.empDetailsArray[0].IsMultipleSupv)
      return (
        <View style={{ flex: 0 }}>
          <LabelTextNoValue heading={globalConstants.EMPLOYEE_TEXT} description={empName} />
          
           {empDetailsArray[0].IsMultipleSupv ?null : <LabelTextNoValue heading={globalConstants.APPROVER_TEXT} description={approveName} />}
          
          
          <LabelTextNoValue heading={globalConstants.COMPANY_CODE_TEXT} description={compName} />
          <LabelTextNoValue heading={globalConstants.STATUS_TEXT} description={statusDesc} />
        </View>
      );
    }
  ...
  calenderView = () => {
    ...
    return (
      <View style={styles.topViewStyle1}>
        <View style={{ flex: 0, flexDirection: 'row', paddingHorizontal: 1, marginTop: 4 }}>
          <View style={{ flex: 1, flexDirection: 'row', borderWidth: 1, borderColor: appConfig.FIELD_BORDER_COLOR }}>
            <Icon style={{ flex: 0 }} name={'calendar-month-outline'} size={30} color={appConfig.BLUISH_COLOR} />
              <Dropdown
              forwardedRef={this.yearRef}
              dropDownData={yearData}
              dropDownCallBack={(index, value) => this.onYearSelection(index, value)}
            />
          </View>
        </View>
        <View style={{ flex: 0, flexDirection: 'row', marginRight: 2, marginTop: 2, borderWidth: 1, borderColor: appConfig.FIELD_BORDER_COLOR }}>
          <Icon style={{ flex: 0 }} name={'calendar-clock'} size={30} color={appConfig.BLUISH_COLOR} />
            <Dropdown
              forwardedRef={this.weekCalenderRef}
              dropDownData={weekArray.map(it => it.Display)}
              dropDownCallBack={(index, value) => this.onWeekSelection(index, value)}
            />
        </View>
      </View>
    );
  ...
  updateSearch = (text) => {
    ...
    if (text) {
      ...
      const newData = this.state.getSupervisors.filter(
        function (item) {
...
approverSearch  =( ) =>{
  ...
  >
    <View style={styles.modalContainer}>
      <View style={styles.searchHolder}>
        ...
        data={this.state.filteredData && this.state.filteredData.length > 0 ? this.state.filteredData : this.state.getSupervisors}
  ...
  approverList = (key) =>{
    ...
    return (
       <View key ={key}>
        ...
        <Text  style={[styles.textInputStyle,{paddingLeft:5}]}>{this.state.selectedApprover ? this.state.selectedApprover :"Select Approver"}</Text>
            ...
            <CustomButton
              ...
              performAction={() => {
                ...
                return alert('Please select week.');
                 ...
                 { return alert('Could not fetched emp data from server, please try again after some time.');
                 }
  ...
  weekStatusView = (heading1, heading2, myColor, item) => {
    ...
    borderColor:'#C9C9C9',
          ...
          <View style={{justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 6, flexDirection: 'row'}}>
            ...
            <View style={{ marginLeft: 2, flexDirection: 'row', marginRight: 6, justifyContent: 'center', alignItems: 'center' }}>
            {item?.TID ?
            <View style={{  borderLeftWidth: 1, marginLeft: 6, paddingLeft: 6}}>
            <TouchableOpacity onPress={() => this.fetchHistory(item)}>
              <Icon style={{ flex: 0 }} name="history" size={30} color={appConfig.DARK_BLUISH_COLOR} />
            </TouchableOpacity>
            </View>
            : null}
          </View>
  ...
  statusColor = (status) => {
    if (status == 1) { //pending with staff
      return 'rgb(96, 130, 182)';
    }
    if (status == 2) { //pending with approver
      return 'rgb(216,0,12)';
    }
  ...
  lastThreeWeekStatusView = () => {
     const checkMultiSup = this.state.empDetailsArray.map((item,key)=>item.IsMultipleSupv)
  //  console.log("getdta",checkMultiSup[0])
    ...
    return (
      <View>
          ...
          return <TouchableOpacity key={'prevWeek_' + idx} onPress={()=> {
        ...
        if(checkMultiSup[0]){
            ...
            {this.weekStatusView(it.Display, it.StatusDesc, this.statusColor(it.Status), it)}
          </TouchableOpacity>;
          })}
      </View>
    );
  ...
  topView = () => {
    return (
      <View style={{ flex: 0, borderWidth: 2, borderColor: 'grey', marginVertical: 3, padding: 3 }} >
        {this.employeeDetailsView()}
        {this.calenderView()}
        {this.state.empDetailsArray.map((item,key)=>item.IsMultipleSupv ? this.approverList(key):null)}
        {this.approverSearch()}
...
export default connect(mapStateToProps, mapDispatchToProps)(MyTimesheetScreen4);
