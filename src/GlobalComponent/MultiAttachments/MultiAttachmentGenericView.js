...
export const MultiAttachmentGenericView = props => {
  ...
  const updateFilesAfterDelete = deletedFile => {
    ...
    let removeIndex =  lineItemArray[props.itemIndex].LstUploadFiles.findIndex((item)=>item?.RowId == deletedFile.RowId);
 ...
 useEffect(()=>{
  console.log('image file is : ', props.lineItems[props.itemIndex]?.LstUploadFiles);
 });
  return (
    <View style={styles.container}>
      ...
      {props.lineItems[props.itemIndex]?.LstUploadFiles.length > 0 ? (
        props.lineItems[props.itemIndex].LstUploadFiles.map((imageItem, imageIndex)=>{
         return <View style={{ flex: 1 }}>
          <View style={styles.fileItem} key={imageItem?.RowID}>
                ...
                >
                  <Text style={styles.fileHeadingStyle}>{imageItem?.FileName}</Text>
                </TouchableOpacity>
                {!props.disable ? (
                  <View style={styles.deleteIcon}>
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        deleteFile(props.itemIndex, imageItem, props.docNumber, props.rowId);
                      }}
                    >
                      <Icon
                        name="delete"
