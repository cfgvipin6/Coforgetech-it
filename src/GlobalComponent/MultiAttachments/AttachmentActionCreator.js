...
import { fetchPOSTMethod, fetchGETMethod,fetchAnotherPost} from '../../utilities/fetchService';
let globalConstants = require('../../GlobalConstants');
let constants = require('./constants');
import {showToast} from '../../GlobalComponent/Toast';

export const deleteAttachment = (loginData,docNumber,rowId,setLoading,updateFiles,file)=>{
    ...
    export const downloadAttachment = (loginData,ErowId,setLoading,openDownloadedFile)=>{
      return async dispatch => {
          let isNetwork = await netInfo();
          if (isNetwork) {
            try {
...
export const AttachmentUpload = (loginData,docNumber,ItemID,File,setData,res,setLoading,updateFiles, docType)=>{
    return async dispatch => {
        let isNetwork = await netInfo();
        if (isNetwork) {
          try {
    ...
    export const AttachmentUploadLTA = (loginData,docNumber,File,setData,res,setLoading,updateFiles)=>{
      return async dispatch => {
          let isNetwork = await netInfo();
          if (isNetwork) {
            try {
