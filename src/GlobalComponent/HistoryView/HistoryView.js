    ...
    >
      <View style={styles.panelNewContainer}>
        <View style={{width:'100%'}}>
          <TouchableOpacity style={{alignSelf:'flex-end',marginTop:10,marginRight:10}} onPress={() => onClosure(props)}>
          <Image
        ...
        >
         ...
         <View style={{flexDirection:'row',alignSelf:'center',borderColor:'light-grey',borderBottomWidth:0.25,width:'96%',paddingVertical:5,marginLeft:5,zIndex:10,borderBottomColor:'light-grey'}}>
          ...
          <Text style={{color:appConfig.BLUISH_COLOR,marginLeft:10}}>{globalConstants.HISTORY_RECORD_HEADING_TEXT + `${index + 1}`}</Text>
         </View>
...
const renderCardItem = (data, props) => {
  ...
  if (props.isComingFromVoucher) {
     var toFb = data.FromEmpBand.trim()!= "B" ? " - BAND " + data.FromEmpBand : ' - ORGKEY ' + data.FromEmpBand;
    //  var fromBand = data.FROMEMPName.trim() != "" ? data.FromStatus.trim() != "3" ? toFb : '' : '';
      var toB = data.ToEmpBand.trim() != "B" ? ' - BAND ' + data.ToEmpBand : " - ORGKEY " + data.ToEmpBand;
      // var  toBand = data.ToEmpName.trim() !=  "" ? data.ToStatus.trim() != "3" ? toB : '' : "";
    return (
      <View>
        {renderRowData(
          constants.FROM_EMP_TEXT,
          data.EmpFrom.concat(" : ").concat(data.FROMEMPName.trim()).concat(toFb  ? toFb : toB)
    ...
    return (
      <View>
        {renderRowData(constants.FROM_EMP_TEXT, data.pendingFrom || data.FromEmployee)}
        {renderRowData(constants.TO_EMP_TEXT, data.pendingTo || data.ToEmployee)}
